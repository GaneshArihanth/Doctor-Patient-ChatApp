...
const DoctorDashboard: React.FC = () => {
    ...
    const fetchPatients = async () => {
      try {
        ...
        const patientsList = conversations.map((conversation: any) => {
          const lastSeenDate = conversation.user.lastSeen
            ? parseISO(conversation.user.lastSeen)
            : null;
          const lastMessageDate = conversation.lastMessage?.createdAt
            ? parseISO(conversation.lastMessage.createdAt)
            : null;
          ...
          return {
            ...
            lastSeen:
              lastSeenDate && isValid(lastSeenDate)
                ? format(lastSeenDate, "p, dd/MM/yy")
                : "N/A",
  ...
  const handleAvailabilityChange = async (
    ...
    try {
      await usersAPI.updateAvailability(newStatus);
    } catch (error) {
      console.error("Error updating availability:", error);
